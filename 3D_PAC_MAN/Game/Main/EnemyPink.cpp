...
//コンストラクタ
CEnemyPink::CEnemyPink(D3DXVECTOR3 Pos, D3DXVECTOR3 Angle, D3DXVECTOR3 Scale)
{
	m_vPos = Pos;
	m_vAngle = Angle;
	m_vScale = Scale;
...
void CEnemyPink::Init()
{
	...
	//モデル
	m_pMesh = g_Loader.GetMesh(Model_Enemy_Pink);
	...
	//休憩地点
	m_vRestPoint[0] = D3DXVECTOR2(1.0f, 5.0f);
	m_vRestPoint[1] = D3DXVECTOR2(1.0f, 1.0f);
	m_vRestPoint[2] = D3DXVECTOR2(8.0f, 1.0f);
	m_vRestPoint[3] = D3DXVECTOR2(8.0f, 3.0f);
...
void CEnemyPink::Move()
{
	...
	if ((int)m_vPos.x % 4 == 0 && (int)m_vPos.z % 4 == 0)
	{
		...
		else
		{
			...
			//プレイヤーの移動ベクトル取得
			D3DXVECTOR3 PlayerMove = m_pPlayer->GetDir();
			...
			{
				//３マス先に設定
				Goal_x += (int)PlayerMove.z * 3;
				Goal_y += (int)PlayerMove.x * 3;
